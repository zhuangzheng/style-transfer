...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.transpose(tf.reshape(tf.transpose(a_C),[n_C,-1]))
    a_G_unrolled = tf.transpose(tf.reshape(tf.transpose(a_G),[n_C,-1]))
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = 1/(pow(2*n_C*n_H*n_W,2)) * tf.reduce_sum(tf.square(GS - GG))
